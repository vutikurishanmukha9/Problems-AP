...
* **Strict 2-Feature Civic Scope**:
  1. **Report a Problem**: Submit public issues (roads, drinking water, drainage, electricity, sanitation, etc.) in under 1 minute with optional photos and GPS coordinates. Automatically receives an anonymous Reference ID (e.g., `AP-2026-6870`).
...
| **Districts** | **28** | All state districts mapped with official administrative headquarters. |
...
│   │   │   ├── taxonomy.ts        # 57 Ministries with Minister Garu & 28 Districts
│   │   │   └── problems.ts        # Problem types and initial dataset
...
* `POST /api/v1/problems` — Submit a new anonymous citizen problem.
...
* `GET /api/v1/taxonomy/districts` — Complete list of 28 districts with headquarters.
